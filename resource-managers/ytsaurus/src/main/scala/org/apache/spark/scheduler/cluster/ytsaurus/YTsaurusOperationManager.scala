...
private[spark] class YTsaurusOperationManager(
  ...
  def setOperationDescription(
    operation: YTsaurusOperation, description: Map[String, String]): Unit = {
  ...
  private def createSpec(
    conf: SparkConf, taskName: String, opParams: OperationParameters): VanillaSpec = {
  ...
  private def executorParams(
    ...
    OperationParameters(spec, conf.get(Config.MAX_EXECUTOR_FAILURES) * numExecutors, attemptId)
  }
}
...
private[spark] object YTsaurusOperationManager extends Logging {
  ...
  def create(
    ytProxy: String, conf: SparkConf, networkName: Option[String]): YTsaurusOperationManager = {
    ...
    try {
      ...
      val portoLayers =
        releaseConfig.getListO("layer_paths").orElse(YTree.listBuilder().buildList())
      val filePaths = releaseConfig.getListO("file_paths").orElse(YTree.listBuilder().buildList())
      val pythonPaths =
        globalConfig.getMapO("python_cluster_paths").orElse(YTree.mapBuilder().buildMap())
      ...
      val distrRootPath = Seq(
        conf.get(SPARK_DISTRIBUTIVES_PATH), svMajor, svMinor, svPatch).mkString("/")
      val distrTgzOpt = Some(distrRootPath)
        .filter(path => ytClient.existsNode(path).join()).flatMap { path =>
      ...
      distrTgzOpt match {
        case Some(sparkTgz) =>
          filePaths.add(YTree.stringNode(s"$distrRootPath/${sparkTgz.stringValue()}"))
      ...
      val prepareEnvCommand =
        s"./setup-spyt-env.sh --spark-home $home --spark-distributive $sparkTgz"
  ...
  private[ytsaurus] def applicationFiles(conf: SparkConf): Seq[String] = {
    ...
    (providedLists ++ primaryResourceSeq)
      .filter(_.startsWith("yt:/")).map(YtWrapper.formatPath).distinct
  }
      ...
      enablers.keys().forEach { enabler =>
        if (conf.contains(enabler)) {
          ...
          if (confValue != updatedValue) {
            logWarning(
              s"Property $enabler was explicitly set to $updatedValue because of cluster settings"
            )
  ...
  def getWebUIAddress(operation: YTreeNode): Option[String] = {
    val description =
      WEB_UI_PATH.foldLeft(Some(operation).asInstanceOf[Option[YTreeNode]]) { (yTreeOpt, key) =>
  ...
  private def buildClient(
    ytProxy: String, user: String, token: String, networkName: Option[String]): YTsaurusClient = {
...
private[spark] case class OperationParameters(
  taskSpec: Spec,
